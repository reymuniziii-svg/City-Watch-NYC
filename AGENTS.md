...
Guidance for AI coding assistants (Codex, Cursor, etc.) working in this repository.
...
Work happens on feature branches named `Codex/<feature>-<suffix>` branched from `main`. The current branch is tracked in the task description. Push to that branch; do not push to `main` without explicit permission.
